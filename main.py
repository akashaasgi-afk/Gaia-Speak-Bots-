...
# --- CONFIGURATION (APKA NAYA ADDRESS) ---
...
CERBERUS_SYSTEM = f"""You are CERBERUS. Technical Guardian. 
DEPLOYED ADDRESS: {CONTRACT_ADDRESS}
CAPABILITIES: Solidity, Hardhat, Chainlink. Always reply in user's language.
[IDENTIFIER: CERBERUS]"""

LILITH_SYSTEM = """You are LILITH. Deployment Manager. 
CAPABILITIES: GitHub Sync, Supply Chain, Gmail Outreach.
[IDENTIFIER: LILITH]"""

app = Flask(__name__)

# HTML UPDATED WITH ADDRESS AND SILVER SECTION
HTML = """<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        .tab{flex:1;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#666;font-size:13px;text-transform:uppercase;}
        ...
        .inp-bar{position:fixed;bottom:0;width: 100%;padding: 10px;background:#111;display:flex;gap:8px;border-top:1px solid var(--border);box-sizing: border-box;}
        input{flex:1;background:#000;border:1px solid var(--border);color:#fff;padding:12px;border-radius:4px;}
        button{background:none;border:2px solid var(--gold);color:var(--gold);padding:10px;cursor:pointer;font-weight:bold;}
        .addr-box{font-size:10px; color:var(--success); background:rgba(0,255,136,0.1); padding:5px; border-radius:4px; margin-top:5px; border:1px solid var(--success);}
    </style>
</head>
...
<div id="brief-v" class="view" style="display:none; padding:20px;">
    <div style="background:#111; border:1px solid var(--border); padding:15px; border-radius:8px;">
        ...
        <button onclick="window.open('https://github.com/MuhammadIsmailIsmail/')" style="width:100%;">SYNC WITH GITHUB</button>
    </div>
</div>

<div class="inp-bar">
    <input id="u-i" placeholder="Command Cerberus or Lilith..." onkeypress="if(event.key==='Enter')sd()">
    <button onclick="sd()">EXECUTE</button>
</div>
...
<script>
    ...
    async function sd(){
        ...
        d.className='m user'; d.innerText=v; box.appendChild(d);
        box.scrollTop=box.scrollHeight;
